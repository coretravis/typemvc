...
describe('@url', () => {
  ...
  it('fails for http:// with no host', () => {
    const [v] = getValidators(proto, 'link');
    expect(v?.('http://', {}).isValid).toBe(false);
  });

  it('fails for a javascript: scheme', () => {
    const [v] = getValidators(proto, 'link');
    expect(v?.('javascript:alert(1)', {}).isValid).toBe(false);
  });

  it('fails for a value with a space in the host', () => {
    const [v] = getValidators(proto, 'link');
    expect(v?.('http://exa mple.com', {}).isValid).toBe(false);
  });
});
...
describe('@pattern', () => {
  ...
  it('validates a global regex consistently across repeated calls', () => {
    class GlobalPatternDto {
      @pattern(/^\d+$/g)
      code = '';
    }
    const [v] = getValidators(GlobalPatternDto.prototype, 'code');
...
// ---------------------------------------------------------------------------
// getAllValidatedFields inheritance
// ---------------------------------------------------------------------------

describe('getAllValidatedFields inheritance', () => {
  it('includes fields declared on a base class', () => {
    class BaseFields {
      @dataType('string')
      @required()
      name = '';
    ...
    const fields = getAllValidatedFields(ChildFields.prototype);
    expect(fields).toContain('name');
    expect(fields).toContain('age');
  });
});
...
describe('coerceToType', () => {
  describe("'string'", () => {
    ...
    it('accepts an absent value', () => {
      const r = coerceToType(null, 'string');
      expect(r.ok).toBe(true);
      if (r.ok) expect(r.value).toBeNull();
    });

    it('rejects a non-string value such as a File', () => {
      const file = new File(['data'], 'photo.png', { type: 'image/png' });
      const r = coerceToType(file, 'string');
