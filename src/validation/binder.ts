import { getAllValidatedFields, getDataType, getDataTypeMessage, getValidators, coerceToType, requiredError } from './decorators.js';
...
export function bindFormData(
  ...
  for (const field of fields) {
    ...
    // Presence is tested on the raw value before coercion: a missing required field
    // fails here rather than coercing to a value (0, false) that satisfies required.
    const requiredMessage = requiredError(proto, field, rawValue);
