...
import type { LayoutMap } from '../../src/layout/layout.js';
...
// ---------------------------------------------------------------------------
// Linking a parent after creation, which is how a layout file's @parent is wired
// ---------------------------------------------------------------------------

describe('_setLayoutParent: linking layouts after they are created', () => {
  it('a linked parent joins the chain', () => {
    const App = defineLayout({ template: (ctx) => html`<app>${ctx.slot}</app>` });
    const Admin = defineLayout({ template: (ctx) => html`<admin>${ctx.slot}</admin>` });
    _setLayoutParent(Admin, App);

    @controller('/linked-parent')
    @layout(Admin)
    class LinkedCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = LinkedCtrl as unknown as new (...args: unknown[]) => unknown;
    const chain = resolveLayoutChain(cls, 'index');
    expect(chain).toEqual([Admin, App]);
  });

  it('a linked chain renders inside out', () => {
    const App = defineLayout({ template: (ctx) => html`<app>${ctx.slot}</app>` });
    const Admin = defineLayout({ template: (ctx) => html`<admin>${ctx.slot}</admin>` });
    _setLayoutParent(Admin, App);

    const result = applyLayoutChain([Admin, App], html`<p>page</p>`, makeContext());
    expect(fragmentHtml(result)).toMatch(/<app><admin><p>page<\/p><\/admin><\/app>/);
  });

  it('resolves a three level chain in order, innermost first', () => {
    const Root = defineLayout({ template: (ctx) => html`<root>${ctx.slot}</root>` });
    const App = defineLayout({ template: (ctx) => html`<app>${ctx.slot}</app>` });
    const Admin = defineLayout({ template: (ctx) => html`<admin>${ctx.slot}</admin>` });
    _setLayoutParent(App, Root);
    _setLayoutParent(Admin, App);

    @controller('/three-level')
    @layout(Admin)
    class ThreeLevelCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = ThreeLevelCtrl as unknown as new (...args: unknown[]) => unknown;
    const chain = resolveLayoutChain(cls, 'index');
    expect(chain).toEqual([Admin, App, Root]);

    const result = applyLayoutChain(chain, html`<p>page</p>`, makeContext());
    expect(fragmentHtml(result)).toMatch(
      /<root><app><admin><p>page<\/p><\/admin><\/app><\/root>/,
    );
  });

  it('a layout with no parent still resolves to a single element chain', () => {
    const Solo = defineLayout({ template: (ctx) => html`<solo>${ctx.slot}</solo>` });

    @controller('/unlinked')
    @layout(Solo)
    class UnlinkedCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = UnlinkedCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(resolveLayoutChain(cls, 'index')).toEqual([Solo]);
  });

  it('links a layout declared in TypeScript as the parent of one linked by name', () => {
    const Programmatic = defineLayout({ template: (ctx) => html`<prog>${ctx.slot}</prog>` });
    const FromFile = defineLayout({ template: (ctx) => html`<file>${ctx.slot}</file>` });
    _setLayoutName(FromFile, 'FromFile');
    _setLayoutParent(FromFile, Programmatic);

    const result = applyLayoutChain([FromFile, Programmatic], html`<p>page</p>`, makeContext());
    expect(fragmentHtml(result)).toMatch(/<prog><file><p>page<\/p><\/file><\/prog>/);
  });

  it('accepts a layout linked by name as the parent of one defined in TypeScript', () => {
    const FromFile = defineLayout({ template: (ctx) => html`<file>${ctx.slot}</file>` });
    _setLayoutName(FromFile, 'FromFile');
    const Programmatic = defineLayout({
      parent: FromFile,
      template: (ctx) => html`<prog>${ctx.slot}</prog>`,
    });

    @controller('/prog-child')
    @layout(Programmatic)
    class ProgChildCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = ProgChildCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(resolveLayoutChain(cls, 'index')).toEqual([Programmatic, FromFile]);
  });

  it('an action level layout brings its own parent chain, overriding the controller level one', () => {
    const App = defineLayout({ template: (ctx) => html`<app>${ctx.slot}</app>` });
    const Admin = defineLayout({ template: (ctx) => html`<admin>${ctx.slot}</admin>` });
    const Portal = defineLayout({ template: (ctx) => html`<portal>${ctx.slot}</portal>` });
    _setLayoutParent(Admin, App);

    @controller('/action-override-chain')
    @layout(Portal)
    class OverrideChainCtrl extends Controller {
      @layout(Admin)
      @get('report')
      report(): unknown { return null; }

      @get()
      index(): unknown { return null; }
    }

    const cls = OverrideChainCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(resolveLayoutChain(cls, 'report')).toEqual([Admin, App]);
    expect(resolveLayoutChain(cls, 'index')).toEqual([Portal]);
  });

  it('throws when the child was not created by defineLayout', () => {
    ...
    expect(() => {
      _setLayoutParent(NotALayout, App);
    }).toThrow('[TypeMVC]');
  });
});
...
describe('resolveLayoutChain: cyclic parent chain', () => {
  it('names the layouts in the cycle', () => {
    const A = defineLayout({ template: (ctx) => html`<a>${ctx.slot}</a>` });
    const B = defineLayout({ template: (ctx) => html`<b>${ctx.slot}</b>` });
    _setLayoutName(A, 'AdminLayout');
    _setLayoutName(B, 'AppLayout');
    _setLayoutParent(A, B);
    _setLayoutParent(B, A);

    @controller('/cycle-named')
    @layout(A)
    class CycleCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = CycleCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(() => resolveLayoutChain(cls, 'index')).toThrow(
      '[TypeMVC] Circular layout chain: AdminLayout -> AppLayout -> AdminLayout.',
    );
  });

  it('throws for a layout that is its own parent', () => {
    const Self = defineLayout({ template: (ctx) => html`<s>${ctx.slot}</s>` });
    _setLayoutName(Self, 'SelfLayout');
    _setLayoutParent(Self, Self);

    @controller('/cycle-self')
    @layout(Self)
    class SelfCycleCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = SelfCycleCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(() => resolveLayoutChain(cls, 'index')).toThrow(
      'SelfLayout -> SelfLayout',
    );
  });

  it('throws for a cycle between layouts that carry no name', () => {
    const A = defineLayout({ template: (ctx) => html`<a>${ctx.slot}</a>` });
    const B = defineLayout({ template: (ctx) => html`<b>${ctx.slot}</b>` });
    _setLayoutParent(A, B);
    _setLayoutParent(B, A);

    @controller('/cycle-unnamed')
    @layout(A)
    class UnnamedCycleCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = UnnamedCycleCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(() => resolveLayoutChain(cls, 'index')).toThrow('[TypeMVC] Circular layout chain');
  });

  it('resolves a layout referenced by name through the layout map', () => {
    const App = defineLayout({ template: (ctx) => html`<app>${ctx.slot}</app>` });
    const Admin = defineLayout({ template: (ctx) => html`<admin>${ctx.slot}</admin>` });
    _setLayoutParent(Admin, App);
    const map: LayoutMap = { AdminLayout: Admin, AppLayout: App };

    @controller('/by-name')
    @layout('AdminLayout')
    class ByNameCtrl extends Controller {
      @get()
      index(): unknown { return null; }
    }

    const cls = ByNameCtrl as unknown as new (...args: unknown[]) => unknown;
    expect(resolveLayoutChain(cls, 'index', map)).toEqual([Admin, App]);
  });
});
