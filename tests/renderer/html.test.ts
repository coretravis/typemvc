...
describe('raw text element interpolation', () => {
  it('html throws for a hole inside <textarea>', () => {
    expect(() => html`<textarea>${'hello'}</textarea>`).toThrow(
      /\[TypeMVC\] Interpolation inside <textarea>/u,
    );
  });

  it('html throws for a hole inside <script>', () => {
    expect(() => html`<script>${'x'}</script>`).toThrow(
      /\[TypeMVC\] Interpolation inside <script>/u,
    );
  });
});

describe('textarea value binding', () => {
  it('binds a signal through the value attribute and updates the property', () => {
    const text = signal('draft');
    const frag = html`<textarea value="${text}" rows="3"></textarea>`;
    const area = frag.nodes[0] as HTMLTextAreaElement;
    ...
    text.set('revised');
    flush();
    expect(area.value).toBe('revised');
  });
});

describe('comments in a template', () => {
  it('binds a node hole that follows a comment containing an apostrophe', () => {
    const frag = html`<!-- the topbar's switcher -->
      <p>${'HELLO'}</p>`;
    expect(outerHtml(frag)).toContain('<p>HELLO</p>');
  });

  it('renders nothing for a hole inside a comment', () => {
    const frag = html`<div><!-- ${'SECRET'} --><p>${'SHOWN'}</p></div>`;
    const div = frag.nodes[0] as Element;
    expect(div.textContent).toBe('SHOWN');
    expect(div.innerHTML).not.toContain('SECRET');
  });
});
