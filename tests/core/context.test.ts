...
// ---------------------------------------------------------------------------
// context.data snapshot and context.model freeze policy
// ---------------------------------------------------------------------------

describe('context.data snapshot', () => {
  it('leaves the source ViewBag mutable after assembly', () => {
    const data = new ContextData();
    data.set('a', 1);
    assembleContext(null, data, makeErrors(), makeRouter(), {}, new URLSearchParams(), {});
    expect(() => { data.set('b', 2); }).not.toThrow();
  });

  it('freezes a copy, so mutating the ViewBag after assembly does not change context.data', () => {
    const data = new ContextData();
    data.set('a', 1);
    const ctx = assembleContext(null, data, makeErrors(), makeRouter(), {}, new URLSearchParams(), {});
    data.set('b', 2);
    expect(ctx.data.a).toBe(1);
    expect(ctx.data.b).toBeUndefined();
    expect(Object.isFrozen(ctx.data)).toBe(true);
  });
});

describe('context.model freeze policy', () => {
  it('freezes the model in place, keeping its identity', () => {
    const model = { name: 'x' };
    const ctx = assembleContext(model, null, makeErrors(), makeRouter(), {}, new URLSearchParams(), {});
    expect(Object.isFrozen(ctx.model)).toBe(true);
