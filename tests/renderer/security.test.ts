...
import { escapeHtml, escapeAttr, sanitizeUrl, sanitizeSrcset } from '../../src/renderer/escape.js';
...
// ---------------------------------------------------------------------------
// sanitizeSrcset -- each candidate URL sanitized
// ---------------------------------------------------------------------------

describe('sanitizeSrcset', () => {
  it('neutralises a dangerous candidate among valid ones', () => {
    const result = sanitizeSrcset('safe.jpg 1x, javascript:alert(1) 2x');
    expect(result).toContain('safe.jpg 1x');
    expect(result).not.toContain('javascript:');
    expect(result).toContain('# 2x');
  });

  it('keeps valid candidates and their descriptors', () => {
    const result = sanitizeSrcset('/a.jpg 480w, /b.jpg 800w');
    expect(result).toBe('/a.jpg 480w, /b.jpg 800w');
  });

  it('sanitizes a dangerous candidate in the first position', () => {
    const result = sanitizeSrcset('javascript:alert(1) 1x, safe.jpg 2x');
    expect(result).not.toContain('javascript:');
    expect(result).toContain('safe.jpg 2x');
  });

  it('sanitizes a bound srcset attribute in a rendered template', () => {
    const evil = 'javascript:alert(1) 1x, /ok.jpg 2x';
    const fragment = html`<img srcset=${evil} />`;
    const img = fragment.nodes[0] as HTMLImageElement;
    expect(img.getAttribute('srcset')).not.toContain('javascript:');
    expect(img.getAttribute('srcset')).toContain('/ok.jpg 2x');
  });
});
...
//
// Both apply their value through a DOM API rather than through markup, so there
// is no HTML escaping question in either position. A CSS value is not sanitized:
// the same value could be written into a style attribute today, and the property
// is only read back by a stylesheet the application itself authored.
// ---------------------------------------------------------------------------

describe('class: and style: binding values', () => {
  it('applies a class token through classList rather than markup', () => {
    const frag = html`<i class:is-active="${true}"></i>`;
    const el = frag.nodes[0] as Element;

    expect(el.classList.contains('is-active')).toBe(true);
    expect(el.hasAttribute('class:is-active')).toBe(false);
  });

  it('does not let a quote in a class binding value escape the attribute', () => {
    const frag = html`<i class:is-active="${'"><script>alert(1)</script>'}"></i>`;
    const el = frag.nodes[0] as Element;
  ...
  it('does not let markup in a style binding value reach the DOM as markup', () => {
    const frag = html`<div style:--fill="${'"><script>alert(1)</script>'}"></div>`;
    const el = frag.nodes[0] as HTMLElement;

    expect(el.querySelector('script')).toBeNull();
    expect(el.tagName.toLowerCase()).toBe('div');
  });

  it('passes a CSS value to the property verbatim', () => {
    const frag = html`<div style:--bg="${'url(https://example.com/a.png)'}"></div>`;
    const el = frag.nodes[0] as HTMLElement;

    expect(el.style.getPropertyValue('--bg')).toBe('url(https://example.com/a.png)');
  });
});
