...
function describeValidationError(err: TmvcValidationError): string {
  switch (err.kind) {
    ...
    case 'invalid-use-directive':
      return `@use on line ${String(err.line)}: ${err.source}`;
    case 'invalid-parent-directive':
      return `@parent on line ${String(err.line)}: ${err.source}`;
    case 'parent-outside-layout':
      return `@parent outside a layout on line ${String(err.line)}: ${err.source}`;
    case 'local-in-view':
