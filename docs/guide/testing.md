...
| `renderTemplate`, `renderView`, `renderComponent` | Yes | `happy-dom` or `jsdom` |
...
A component is where props, `@local` state, effects, refs, and event handlers
meet, so it is worth a unit test rather than a browser check. `renderComponent`
mounts one and returns the same surface as `renderView`, plus a `container` and
an `unmount()`.

It needs a DOM environment, exactly like the other render helpers:

```ts
// @vitest-environment happy-dom
import { renderComponent, flushEffects } from '@typemvc/core/testing';
import '@typemvc/core/testing/vitest';
import Counter from '../src/components/Counter.tmvc';
```

Take this component, which holds its own state in a `@local` block:

```html
@props { label: string; step: number }
@local {
  const count = signal(0);
  const increment = () => count.update((n) => n + props.step);
...
Its whole behaviour is testable without a browser. Pass the props, drive the
DOM, flush, assert:

```ts
it('counts up by the step and resets', () => {
  const { query, click, unmount } = renderComponent(Counter, { label: 'Hits', step: 2 });

  expect(query('.counter__label')).toHaveText('Hits');
...
This is the level a bug report should arrive at: a failing test against the
component, not a screenshot of the page it sits on.

Props are passed as given. The helper does not default or validate them; the
props contract is enforced at the call site by the compiler and the editor
plugin.

The component is mounted, so `ref` callbacks fire and receive a connected
element, as they do in the browser:

```ts
const { container } = renderComponent(SearchBox);
expect(document.activeElement).toBe(container.querySelector('input'));
```

`unmount()` disposes the component and empties the container. Everything the
render registered is torn down: `@local` effects stop, and `onCleanup` callbacks
run. Assert that, because a component that leaks an interval or a document
listener does so in production too:

```ts
it('stops its clock when it goes away', () => {
  const { query, unmount } = renderComponent(Clock, { tz: 'UTC' });
  expect(query('.clock')).not.toBeNull();

  unmount();

  expect(query('.clock')).toBeNull();      // the container is empty
  expect(clearInterval).toHaveBeenCalled(); // onCleanup ran
});
```

### Nested component tags

A `<Pill />` tag inside a component's template compiles to a registry lookup by
tag name. In the application, `bootstrap()` fills that registry from the
components glob. A unit test has no bootstrap, so an unregistered tag renders
nothing and warns. Register the components the template under test uses.

Scope the registration to one render with the `components` option, and it is
removed again on `unmount()`:

```ts
const { queryAll, unmount } = renderComponent(
  BookCard,
...
Or register once for the file with `registerComponents`. The registry is module
level state, so a leaked registration is a dependency between tests: clear it in
an `afterEach`.

```ts
import { registerComponents, resetComponents } from '@typemvc/core/testing';

afterEach(resetComponents);

it('renders a pill per tag', () => {
  registerComponents({ Pill, Avatar });

  const { queryAll } = renderComponent(BookCard, { title: 'Dune', tags: ['sci-fi', 'classic'] });
  expect(queryAll('.pill')).toHaveLength(2);
});
```

`registerComponents` merges into the current registry, so it can be called more
than once; `resetComponents()` clears everything it registered.
...
// Components (DOM required)
renderComponent(component, props?, { components? }) -> RenderedComponent
  // RenderedView surface, plus: container, unmount()
registerComponents({ Tag: Component }) -> void
