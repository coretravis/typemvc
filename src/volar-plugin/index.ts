...
// The runtime values a view or a component template may reference. A view is a
// derivation of its model, so it gets `computed` and the output helpers, but not
// `signal` or `effect`: owning state is what a component @local block is for, and
// withholding them here is what makes their misuse in a view a clear diagnostic.
const CORE_VALUE_IMPORT =
  "import { html, svg, computed, keyed, keyedMap, safeHtml, stop, prevent } from '@typemvc/core';\n";
...
// lifted statements type-check against the state primitives. `computed` and the
// output helpers already arrive with CORE_VALUE_IMPORT, so only the state owning
// primitives are added here to avoid a duplicate import. The headless behaviours
// come from the separate behaviors entry point, so their use in a view (which has
// no @local block, and so no reactivity preamble) is an undefined name.
const PREAMBLE_REACTIVITY =
  "import { signal, effect, batch, onCleanup, useForm } from '@typemvc/core';\n" +
  "import { persisted, mediaQuery, hotkey, clickOutside } from '@typemvc/core/behaviors';\n";

// Declared in a virtual file that contains a ref attribute. The tag argument of
// each generated __tmvcRef call names the element the ref is written on, so the
// callback parameter resolves to that element's interface.
const PREAMBLE_REF =
  'type __TmvcRefEl<K extends string> = K extends keyof HTMLElementTagNameMap\n' +
  '  ? HTMLElementTagNameMap[K]\n' +
  '  : K extends keyof SVGElementTagNameMap\n' +
  '    ? SVGElementTagNameMap[K]\n' +
  '    : Element;\n' +
  'declare function __tmvcRef<K extends string>(\n' +
  '  callback: (element: __TmvcRefEl<K>) => (() => void) | void,\n' +
  '): void;\n';
...
// ref callback typing
//
// A `ref` attribute on a native tag is a callback that receives that element, so
// the callback's parameter type is decided by the tag it is written on. The
// expression is reproduced in the non-executed check block inside a call whose
// tag argument names the element, which is what gives the parameter its type.
// ---------------------------------------------------------------------------

/** A `ref="${...}"` attribute on a native tag, with the tag it is written on. */
interface RefUsage {
  /** The lowercased tag name the ref is written on. */
  readonly tag: string;
  /** The callback expression source. */
  readonly exprJs: string;
  readonly exprSourceOffset: number;
  readonly exprLength: number;
}

// Consumes an attribute value starting at the opening quote. Returns the whole
// value hole when the value is exactly one ${...} expression, else null, plus the
// index past the closing quote.
function scanAttrValue(
  source: string,
  quoteIndex: number,
): { hole: { inner: string; innerStart: number } | null; end: number } | null {
  ...
  if ((source[i] ?? '') === '$' && (source[i + 1] ?? '') === '{') {
    ...
    if ((source[span.end] ?? '') === quote) {
      return { hole: { inner: span.inner, innerStart: span.innerStart }, end: span.end + 1 };
    }
  }
  ...
  while (i < n) {
    ...
    if (ch === quote) return { hole: null, end: i + 1 };
    i++;
  }
  return null;
}
...
/**
 * Scans native tags in markup for `ref="${...}"` attributes. Refs inside ${...}
 * expressions are skipped: their expression only type-checks in the scope of the
 * enclosing callback, which the flat check block does not reproduce. A ref whose
 * value is not a single whole expression is skipped too, because the renderer
 * rejects it at run time.
 */
function scanRefUsages(source: string): RefUsage[] {
  const usages: RefUsage[] = [];
  const n = source.length;
  let i = 0;
  ...
  while (i < n) {
    ...
    let j = i + 1;
    let tag = '';
    while (j < n && /[a-zA-Z0-9-]/u.test(source[j] ?? '')) {
      tag += source[j] ?? '';
      j++;
    }
    tag = tag.toLowerCase();

    // Walk this tag's attributes to its end.
    while (j < n) {
      const c = source[j] ?? '';
      if (c === '>') {
        j++;
      ...
      let attrName = '';
      while (j < n && /[a-zA-Z0-9_:-]/u.test(source[j] ?? '')) {
        attrName += source[j] ?? '';
        j++;
      }
      while (j < n && /[ \t]/u.test(source[j] ?? '')) j++;
      if ((source[j] ?? '') !== '=') continue;
      j++;
      while (j < n && /[ \t]/u.test(source[j] ?? '')) j++;
      const value = scanAttrValue(source, j);
      if (value === null) break;
      if (attrName.toLowerCase() === 'ref' && value.hole !== null) {
        usages.push({
          tag,
          exprJs: value.hole.inner,
          exprSourceOffset: value.hole.innerStart,
          exprLength: value.hole.inner.length,
        });
      }
      j = value.end;
    }
...
// Emits one typed call per ref usage. `exprs` offsets are relative to `text`.
function buildRefChecks(usages: readonly RefUsage[]): { text: string; exprs: RepMapping[] } {
  const exprs: RepMapping[] = [];
  let text = '';
  for (const usage of usages) {
    const prefix = `    __tmvcRef<'${usage.tag}'>(`;
    exprs.push({
      genRel: text.length + prefix.length,
      srcOffset: usage.exprSourceOffset,
      len: usage.exprLength,
    });
    text += prefix + usage.exprJs + ');\n';
  }
  return { text, exprs };
}
...
// Reproduces a component @local block's statements with each component tag inside
// an html`` or svg`` literal rewritten to a typed __Cmp_Name({...}) call, so the
// props are checked in the statements' own scope (a `.map()` callback keeps its
// loop variable). The reproduction is placed in the shared `if (false)` block in
// a nested scope, shadowing the lifted statements without conflict. `base` is the
// source offset the statements begin at, so mappings resolve to the .tmvc file.
function buildLocalChecks(
  ...
  let rep: ReproducedExpr | null;
  try {
    rep = reproduceExpr(statements, base, importable);
  } catch {
    rep = null;
  }
  if (rep === null || rep.usedCount === 0) {
  ...
  for (const nm of rep.names) names.add(nm);
  const prefix = '  {\n';
  const exprs: RepMapping[] = rep.exprs.map((m) => ({
    genRel: prefix.length + m.genRel,
    srcOffset: m.srcOffset,
    len: m.len,
  }));
  return { text: prefix + rep.text + '\n  }\n', exprs, names };
}
...
export function generateVirtualTs(
  ...
  const { body, directive, uses } = extractDirective(source);
  ...
  if (isComponent) {
    ...
    if (localBlock !== null) {
      // Import the state primitives after the shared value import.
      base = base.replace(CORE_VALUE_IMPORT, CORE_VALUE_IMPORT + PREAMBLE_REACTIVITY);
  ...
  // Emit the @use imports so tsc resolves each binding's type, then shift the
  // one absolute offset computed so far (the lifted @local mapping) past them.
  // Every later offset is derived from this injected preamble, so it stays
  // correct without further adjustment.
  if (useImports !== '') {
    preamble = useImports + preamble;
    if (localBlockMapping !== null) {
      localBlockMapping = {
        ...localBlockMapping,
        generatedOffsets: localBlockMapping.generatedOffsets.map(
          (o) => o + useImports.length,
        ),
      };
    }
  }
  ...
  // Call-site checking. Component prop checks in markup are views only; component
  // tags inside a @local block are checked for components; ref checks apply to
  // views and components alike. All are declared before the render function and
  // emitted into one non-executed block before the `return html`.
  const hasImports = componentImports !== undefined && componentImports.size > 0;
  const importableSet = new Set(hasImports ? componentImports.keys() : []);
  const usages = !isComponent && hasImports
    ? scanComponentUsages(body).filter((u) => importableSet.has(u.name))
    : [];
  // Usages inside ${...} loops, checked in their real scope (issue 060).
  const loop: LoopChecks = !isComponent && hasImports
    ? buildLoopChecks(body, importableSet)
    : { text: '', exprs: [], names: new Set<string>() };
  // Usages inside a component @local block, checked in the statements' own scope.
  const local: LoopChecks = isComponent && localBlock !== null && hasImports
    ? buildLocalChecks(localBlock.statements, localBlock.sourceStart, importableSet)
    : { text: '', exprs: [], names: new Set<string>() };
  const refs = scanRefUsages(localBlock !== null ? localBlock.markup : body);
  ...
  if (allNames.size > 0 || refs.length > 0) {
    const RETURN_LINE = '  return html`';
    const header =
      [...allNames]
        .map((nm) => `import __Cmp_${nm} from '${componentImports?.get(nm) ?? ''}';\n`)
        .join('') + (refs.length > 0 ? PREAMBLE_REF : '');

    const fnIdx = preamble.indexOf('export default function render');
    if (fnIdx !== -1) {
      preamble = preamble.slice(0, fnIdx) + header + preamble.slice(fnIdx);
      // The lifted @local statements sit after the render signature, so the header
      // shifts their generated position.
      if (localBlockMapping !== null) {
        localBlockMapping = {
          ...localBlockMapping,
          generatedOffsets: localBlockMapping.generatedOffsets.map((o) => o + header.length),
        };
      }
    }

    // Build the flat top-level check block, then splice the loop, @local, and ref
    // checks in before its closing brace so they all sit in one `if (false)` block.
    const { text: topText, exprs, argSpans } = buildCheckBlock(usages);
    const closeIdx = topText.lastIndexOf('  }\n');
    const refChecks = buildRefChecks(refs);
    const checkText =
      topText.slice(0, closeIdx) +
      loop.text +
      local.text +
      refChecks.text +
      topText.slice(closeIdx);
    const localBase = closeIdx + loop.text.length;
    const refBase = localBase + local.text.length;
    ...
    if (retIdx !== -1) {
      ...
      // then the unequal-length argument-object mappings. The loop, @local, and
      // ref mappings are offset by where their text was spliced in.
      extraMappings = [
        ...exprs.map((e) => ({
        ...
        ...local.exprs.map((e) => ({
          sourceOffsets: [e.srcOffset],
          generatedOffsets: [retIdx + localBase + e.genRel],
          lengths: [e.len],
          data: ALL_FEATURES,
        })),
        ...refChecks.exprs.map((e) => ({
          sourceOffsets: [e.srcOffset],
          generatedOffsets: [retIdx + refBase + e.genRel],
          lengths: [e.len],
          data: ALL_FEATURES,
        })),
        ...argSpans.map((a) => ({
 ...
 * and the layouts-only `@parent` rule; the in-block denylist applies regardless.
 * The VS Code extension surfaces these alongside the TypeScript errors derived
...
/**
 * Resolves the components used in a .tmvc file to relative `.tmvc` import paths
 * (component name -> import path). A view resolves every component used in its
 * markup; a component file resolves only the components used inside its @local
 * block, which is the sole place a component file has checked call sites.
 * Unresolved names still get a best-effort path so a "cannot find module" error
 * surfaces rather than a silent skip.
 */
function resolveComponentImports(
  ...
  // A component file only checks tags inside its @local block; a view checks its
  // whole markup, including nested usages inside ${...} loops (issue 060).
  let scanSource = source;
  if (isComponent) {
    const { body } = extractDirective(source);
    const localBlock = extractLocalBlock(body);
    if (localBlock === null) return map;
    scanSource = localBlock.statements;
  }

  const tmvcDir = pathDirname(normalized);
  for (const name of collectComponentNames(scanSource)) {
