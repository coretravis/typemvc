...
describe('AC3: type errors are reported via valid TypeScript structure', () => {
  it('generated virtual file starts with typemvc imports', () => {
    const result = generateVirtualTs(SIMPLE_TMVC, 'views/users/index.tmvc', null);
    expect(result.code.startsWith("import { html, svg, computed, keyed, keyedMap, safeHtml, stop, prevent } from '@typemvc/core';")).toBe(true);
  });

  it('generated virtual file imports html as a value (callable)', () => {
    const result = generateVirtualTs(SIMPLE_TMVC, 'views/users/index.tmvc', null);
    expect(result.code).toContain("import { html, svg, computed, keyed, keyedMap, safeHtml, stop, prevent } from '@typemvc/core';");
  ...
  it('virtual code snapshot getText returns the generated code', () => {
    ...
    expect(fullCode).toContain("import { html, svg, computed, keyed, keyedMap, safeHtml, stop, prevent } from '@typemvc/core';");
...
describe('ref callback typing in the virtual file', () => {
  it('types the callback parameter from the tag it is written on', () => {
    const result = generateVirtualTs(
      '<canvas ref="${(el) => { el.getContext(\'2d\'); }}"></canvas>',
      '/src/views/Home/index.tmvc',
      null,
    );

    expect(result.code).toContain("__tmvcRef<'canvas'>((el) => { el.getContext('2d'); });");
    expect(result.code).toContain('K extends keyof HTMLElementTagNameMap');
    expect(result.code).toContain('declare function __tmvcRef');
  });

  it('maps the ref expression back to its source range', () => {
    const source = '<input ref="${(el) => el.focus()}" />';
    const result = generateVirtualTs(source, '/src/views/Home/index.tmvc', null);
    const expr = '(el) => el.focus()';
    const mapping = result.extraMappings.find((m) => m.lengths[0] === expr.length);

    expect(mapping).toBeDefined();
    const srcStart = mapping?.sourceOffsets[0] ?? 0;
    const genStart = mapping?.generatedOffsets[0] ?? 0;
    expect(source.slice(srcStart, srcStart + expr.length)).toBe(expr);
    expect(result.code.slice(genStart, genStart + expr.length)).toBe(expr);
  });

  it('types refs in a component file, which has no component check block', () => {
    const result = generateVirtualTs(
      '<input ref="${(el) => el.select()}" />',
      '/src/components/Field.tmvc',
      null,
    );

    expect(result.code).toContain("__tmvcRef<'input'>((el) => el.select());");
  });

  it('falls back to the SVG element map for an SVG tag', () => {
    const result = generateVirtualTs(
      '<svg><circle ref="${(el) => el.getBBox()}" /></svg>',
      '/src/views/Home/index.tmvc',
      null,
    );

    expect(result.code).toContain("__tmvcRef<'circle'>((el) => el.getBBox());");
    expect(result.code).toContain('K extends keyof SVGElementTagNameMap');
  });

  it('emits no ref declaration when the template has no ref', () => {
    const result = generateVirtualTs(SIMPLE_TMVC, '/src/views/Home/index.tmvc', null);
    expect(result.code).not.toContain('__tmvcRef');
  });

  it('skips a ref inside an interpolated expression, whose scope it cannot reproduce', () => {
    const result = generateVirtualTs(
      '<ul>${rows.map((r) => html`<li ref="${(el) => el.focus()}">${r}</li>`)}</ul>',
      '/src/views/Home/index.tmvc',
      null,
    );
...
// ---------------------------------------------------------------------------
// Prefixed class and style bindings
//
// The markup is reproduced inside the generated html`...` literal, so the
// expression in a prefixed attribute is an ordinary template expression that
// TypeScript checks in the scope of the render function. The attribute name is
// markup, and a colon in it is nothing the generator has to know about.
// ---------------------------------------------------------------------------

describe('prefixed attribute expressions in the virtual TypeScript', () => {
  it('reproduces a class binding expression inside the checked template', () => {
    const result = generateVirtualTs(
      '<li class="row" class:is-active="${context.data.active}">x</li>',
      '/src/views/Home/index.tmvc',
      null,
    );
    const markup = result.code.slice(result.preambleLength);

    expect(markup).toContain('class:is-active="${context.data.active}"');
    expect(result.code).toContain('return html`');
  });

  it('reproduces a style binding expression inside the checked template', () => {
    const result = generateVirtualTs(
      '<div style:--fill="${context.data.percent}%"></div>',
      '/src/views/Home/index.tmvc',
      null,
    );
    const markup = result.code.slice(result.preambleLength);

    expect(markup).toContain('style:--fill="${context.data.percent}%"');
  });

  it('keeps the markup mapping aligned so a prefixed attribute maps back to source', () => {
    const source = '<li class:is-active="${context.data.active}">x</li>';
    const result = generateVirtualTs(source, '/src/views/Home/index.tmvc', null);
    const sourceOffset = source.indexOf('context.data.active');
    const generatedOffset = result.code.indexOf('context.data.active');

    expect(generatedOffset - result.preambleLength).toBe(sourceOffset);
  });
});
