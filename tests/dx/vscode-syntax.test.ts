...
// ---------------------------------------------------------------------------
// Prefixed attribute names (class:name, style:--prop)
//
// Attribute names are tokenized by text.html.basic, whose attribute-name rule
// accepts a colon, and the expression injection keys on the quoted string scope
// rather than on the attribute the string belongs to. Both are name-agnostic, so
// a prefixed attribute highlights like any other. These pin that the grammar
// keeps delegating and never narrows attribute names to a fixed list.
// ---------------------------------------------------------------------------

describe('prefixed class and style attributes highlight as attributes', () => {
  const attrGrammarRaw = readFileSync(
    join(extRoot, 'syntaxes/tmvc-attribute-expressions.tmLanguage.json'),
    'utf-8',
  );
  ...
  it('the tmvc grammar defines no attribute-name rule of its own', () => {
    const names = Object.keys(grammar.repository);
    expect(names).not.toContain('attribute-name');
    expect(names).not.toContain('tag-stuff');
  });

  it('attribute tokenization is delegated to text.html.basic', () => {
    const includes = grammar.patterns.map((p) => p.include);
    expect(includes).toContain('text.html.basic');
  });

  it('the expression injection selects on the string scope, not on an attribute name', () => {
    const selector = attrGrammar.injectionSelector ?? '';
    expect(selector).toContain('string.quoted.double.html');
    expect(selector).not.toContain('class');
    expect(selector).not.toContain('style');
    expect(selector).not.toContain('entity.other.attribute-name');
  });

  it('the interpolation rule matches an expression in a prefixed attribute value', () => {
    const interp = attrGrammar.repository?.interpolation;
    if (interp?.begin === undefined) return;
    const regex = new RegExp(interp.begin);
    expect(regex.test('${isActive}')).toBe(true);
    expect(regex.test('${percent}%')).toBe(true);
  });
});
