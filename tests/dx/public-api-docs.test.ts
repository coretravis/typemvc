...
const behaviorsEntry = join(repoRoot, 'src', 'behaviors', 'index.ts');
...
function getDocumentedStateOf(entryFile: string): { documented: string[]; undocumented: string[] } {
  ...
  if (sourceFile === undefined) throw new Error(`Could not load ${entryFile}`);
...
describe('behaviors entry API documentation coverage', () => {
  it('every @typemvc/core/behaviors export has a TSDoc comment', () => {
    const { undocumented } = getDocumentedStateOf(behaviorsEntry);
    expect(undocumented, `Undocumented behaviors exports: ${undocumented.join(', ')}`).toEqual([]);
  });

  it('exports the four behaviours', () => {
    const { documented } = getDocumentedStateOf(behaviorsEntry);
    for (const name of ['persisted', 'mediaQuery', 'hotkey', 'clickOutside']) {
