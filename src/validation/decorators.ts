...
/**
 * Finds a field's metadata by walking the prototype chain, so a subclass DTO reads
 * the metadata a base class declared for an inherited field. The nearest definition
 * wins, letting a subclass override an inherited field.
 */
function findFieldMeta(proto: object, propertyName: string): FieldMeta | undefined {
  let current: object | null = proto;
  while (current !== null && current !== Object.prototype) {
    const meta = fieldMetaStore.get(current)?.[propertyName];
...
export function getDataType(proto: object, propertyName: string): DataType | undefined {
  return findFieldMeta(proto, propertyName)?.dataType;
}
...
export function getDataTypeMessage(proto: object, propertyName: string): string | undefined {
  return findFieldMeta(proto, propertyName)?.dataTypeMessage;
}
...
export function getValidators(proto: object, propertyName: string): readonly FieldValidator[] {
  return findFieldMeta(proto, propertyName)?.validators ?? [];
}

/**
 * Returns all property names that carry validation metadata, walking the prototype
 * chain so a subclass DTO includes the fields its base declared. Names from the
 * subclass come first, then the base's, each in source (top-to-bottom) order, with
 * an overridden field listed once.
 */
export function getAllValidatedFields(proto: object): readonly string[] {
  ...
  let current: object | null = proto;
  while (current !== null && current !== Object.prototype) {
    const own = fieldNamesStore.get(current);
    if (own !== undefined) {
...
/** True when a raw form value is missing: absent (null/undefined) or an empty string. */
function isMissingRawValue(rawValue: unknown): boolean {
  return rawValue === null || rawValue === undefined || rawValue === '';
}

/**
 * The required-field error for a raw form value, or undefined when the field is not
 * required or the value is present. Presence is tested on the raw value before
 * coercion, so a missing required field fails rather than coercing to a value that
 * would satisfy the coerced-value required check: a missing number coerces to 0 and
 * a missing checkbox to false, both of which that check accepts.
 */
export function requiredError(proto: object, propertyName: string, rawValue: unknown): string | undefined {
  const meta = findFieldMeta(proto, propertyName);
  if (meta?.required !== true) return undefined;
  // A required checkbox is satisfied only when it is checked, which the boolean
  // coercion recognises as the value 'true' (or a boolean true from a useForm seed).
  // An unchecked box submits nothing, so an unset boolean is missing, not a valid
  // false. Other types are missing when the raw value is absent or an empty string.
  const missing = meta.dataType === 'boolean'
    ? rawValue !== true && rawValue !== 'true'
    : isMissingRawValue(rawValue);
  if (!missing) return undefined;
  return meta.requiredMessage ?? DEFAULT_REQUIRED_MESSAGE;
}
...
export function coerceToType(
  ...
  if (type === 'string') {
    // A form string field must receive text. An absent value is fine (presence is a
    // separate concern), but a File (from a file input) is not a string and must not
    // pass through as one.
    if (rawValue === null || rawValue === undefined || typeof rawValue === 'string') {
      return { ok: true, value: rawValue };
    }
    return { ok: false, error: errorMessage ?? 'Must be text, not a file.' };
  }
...
export function required(errorMessage?: string): PropDec {
  return function (target: object, key: string | symbol): void {
    // The flag lets the model binder and useForm test presence on the raw form
    // value before coercion, so a missing required number does not coerce to 0 and
    // a missing required checkbox does not coerce to false and then satisfy this
    // check. The validator stays for the already-coerced value path.
    const meta = getOrCreateFieldMeta(target, String(key));
    meta.required = true;
    meta.requiredMessage = errorMessage;
    addValidator(target, String(key), (value) => {
      if (value === null || value === undefined || value === '') {
        return ValidationResult.fail(errorMessage ?? DEFAULT_REQUIRED_MESSAGE);
      }
...
/**
 * Requires a valid http or https URL. Parsed with the URL constructor rather than a
 * permissive pattern, so a value with no host, a space, or a non-http scheme (a
...
export function url(errorMessage?: string): PropDec {
  return function (target: object, key: string | symbol): void {
    addValidator(target, String(key), (value) => {
      if (typeof value !== 'string') return ValidationResult.ok();
      const message = errorMessage ?? 'Must be a valid URL starting with http:// or https://.';
      let parsed: URL;
      try {
...
export function pattern(regex: RegExp, errorMessage?: string): PropDec {
  return function (target: object, key: string | symbol): void {
    addValidator(target, String(key), (value) => {
      if (typeof value !== 'string') return ValidationResult.ok();
      // Test with a fresh regex each call: a user-supplied global or sticky regex
      // carries lastIndex between calls, so reusing it would validate the same value
      // inconsistently across repeated submissions.
